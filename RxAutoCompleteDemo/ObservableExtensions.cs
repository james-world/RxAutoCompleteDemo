...
namespace RxAutoCompleteDemo
{
    public static class ObservableExtensions
    {
            ...
            IScheduler scheduler = null)
        {
            ...
            return Observable.Defer<(bool Succeeded, T Value, Exception Exception)>(() =>
            {
                return (++attempt == 1 ? source : source.DelaySubscription(strategy(attempt - 1), scheduler))
                    .Select<T, (bool, T, Exception)>(item => (true, item, null))
                    .Catch<(bool Suceeded, T Value, Exception Exception), Exception>(e => retryOnError(e)
                        ? Observable.Throw<(bool, T, Exception)>(e)
                        : Observable.Return((false, default(T), e)));
            })
            .Retry(retryCount)
            .SelectMany(t => t.Succeeded
                ? Observable.Return(t.Value)
                : Observable.Throw<T>(t.Exception));
        }
    }
}